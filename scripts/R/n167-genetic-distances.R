# =========================================
# Sequence divergence and diversity analysis
# =========================================
...
# -----------------------------
tree <- read.beast("analysis/BEAST_runs/no-trait/n167/strict-constant/n167-strict-constant.hipstr.tre")
phy <- tree@phylo
...
write.csv(results_table,"analysis/genetic_diversity/phylogenetic-cluster-diversity.csv", row.names=F)
# -----------------------------
...
ggplot(plot_df, aes(x = cluster, y = distance, color = cluster)) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  labs(y = "Pairwise genetic distance (SNPs)", x = "", title = "Genetic distances highlighting outliers") +
  theme_minimal() +
...
ggplot(plot_df, aes(x = cluster, y = distance, fill = cluster)) +
  ...
  labs(
    ...
    x = "",
    title = "Genetic distances highlighting outliers"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
